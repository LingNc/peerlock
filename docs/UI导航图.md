...
flowchart TD
    ...
    %% ==================== 管控端主页 ====================
    CH["管控端主页<br/>TopAppBar: 返回箭头 + 设备名 + 设置图标<br/>返回: 设备选择页<br/>StatusCard: 已配对 设备名 策略巡检运行中<br/>验证码每秒刷新: 管理码 解锁码 终止码<br/>(当前设备的3个种子)<br/>策略列表: com.app1 com.app2<br/>[审批请求] Button<br/>[紧急逃生] OutlinedButton<br/>[使用统计] OutlinedButton<br/>[单向控制] OutlinedButton"]
    ...
    subgraph SINGLE_CONTROL ["单向控制页 (管控端)"]
        ...
        SC_ADJUST["调整Tab:<br/>选择目标应用<br/>新时长/时段配置<br/>[生成调整指令]<br/>被控端需确认后执行"]
        SC_STATS["统计Tab:<br/>选择时间范围: 1天/7天/30天/全部<br/>[生成请求指令]<br/>被控端自动发送统计数据"]
    end

    %% ==================== 紧急逃生页 ====================
    subgraph EMERGENCY ["紧急逃生页 (FLAG_SECURE 防截屏)"]
        EM_L1["L1 终止码:<br/>选择要解绑的被控端 (多配对时)<br/>解除配对+清除加密材料 保留DO<br/>[6位终止码] OutlinedTextField<br/>[验证终止码] Button 红色<br/>验证通过: AlertDialog 确认终止<br/>执行后: L1 终止已执行"]
    ...
    %% ==================== 被控端主页 ====================
    CDH["被控端主页<br/>TopAppBar: PeerLock 被控端 + 设置图标<br/>返回: 无 顶级页<br/>StatusCard: 已配对 接受控制方管理<br/>管控方: 设备名 (身份指纹摘要)<br/>受限应用: com.app1 com.app2<br/>[使用统计] OutlinedButton<br/>[发送统计] OutlinedButton<br/>[接收指令] OutlinedButton<br/>[管理策略] OutlinedButton"]
    ...
    subgraph RECEIVE_CMD ["接收指令页 (被控端)"]
        ...
        RC_ADJUST["解析为调整指令:<br/>目标应用: com.app1<br/>新时长/时段: xxx<br/>[确认执行] Button<br/>[拒绝] Button"]
        RC_STATS["解析为统计请求:<br/>时间范围: 最近7天<br/>自动执行: 生成统计报告<br/>展示 QR/字符串 + [复制数据]"]
    end

    %% ==================== 被控端策略管理页 ====================
    subgraph CONTROLLED_POLICY ["策略管理页 (被控端)"]
        CP_LIST["当前策略列表:<br/>com.app1 每日30分钟 09:00-22:00<br/>com.app2 每日60分钟<br/>每项策略右侧 [修改] 按钮"]
    ...
    %% ==================== 设置页 ====================
    SET["设置页<br/>TopAppBar: 返回箭头 + 设置<br/>Device Owner: ✓已设置(灰)/✗未设置(可点击)<br/>电池优化: ✓已关闭(灰)/✗未关闭(可点击)<br/>[配对信息] 点击进入详情页<br/>[配置策略] 默认参数管理<br/>主题模式: [跟随系统] [浅色] [深色] FilterChip<br/>语言: [中文] [English] FilterChip<br/>[切换角色] 回角色选择页(原配置保持运行)<br/>[身份重置] 条件满足时可用 10s确认"]

    %% ==================== 配对信息页 ====================
    PAIR_INFO["配对信息页 (FLAG_SECURE)<br/>当前配对:<br/>  对方设备名: Galaxy S24<br/>  身份指纹: B7:1E:...<br/>  配对时间: 2026-05-02<br/>  会话ID: a3f8-...<br/>  状态: ACTIVE<br/><br/>被控端操作:<br/>  [申请解绑] 输入终止码 直接执行<br/><br/>管控端操作:<br/>  [查看验证码]<br/>  [申请删除种子] 被控端确认后可操作<br/><br/>历史配对记录:<br/>  · Pixel 7 REVOKED 可申请删除种子<br/>  · [归档记录] 查看已归档会话"]

    %% ==================== 配置策略页 ====================
    POLICY_CONFIG["配置策略页<br/>TopAppBar: 返回箭头 + 配置策略<br/>快速码默认解锁时长: 5分钟(可调)<br/>申请模式默认时长: 5分钟(可调)<br/>统计默认附带范围: 最近1天(可调)<br/>(管控端和被控端分别配置)"]

    %% ==================== 申请解绑 (被控端-PAIR_INFO内) ====================
    subgraph UNBIND ["申请解绑 - 在配对信息页内"]
        UNBIND_INPUT["弹出终止码输入框<br/>输入控制方提供的终止码<br/>6位 TotpInputField<br/>验证通过后立刻执行解绑<br/>清除加密材料 返回被控端主页<br/>提示: 配对已解除"]
    end
    ...
    SET -->|配对信息| PAIR_INFO
    SET -->|配置策略| POLICY_CONFIG
    SET -->|切换角色| RS

    %% ==================== 配对信息页 ====================
    PAIR_INFO -.->|返回| SET
    PAIR_INFO -->|申请解绑-被控端| UNBIND_INPUT
    PAIR_INFO -->|历史记录申请删除种子| SD_INFO

    %% ==================== 申请解绑流程 ====================
    UNBIND_INPUT -->|终止码验证通过 立刻执行| CDH
    UNBIND_INPUT -.->|取消| PAIR_INFO
